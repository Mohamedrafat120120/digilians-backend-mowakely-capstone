...
const requestPayoutSchema = Joi.object({
  ...
  method: Joi.string().valid('bank_transfer', 'paypal', 'stripe').optional()
});
...
router.post('/', authenticate, authorize('lawyer'), validate(requestPayoutSchema), payoutController.request);
...
router.patch('/:id/status', authenticate, authorize('admin'), validate(updatePayoutStatusSchema), payoutController.updateStatus);
